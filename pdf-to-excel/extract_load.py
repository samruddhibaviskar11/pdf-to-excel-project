...
attribute_list = ["ID", "cholesterol_v_", "hdl_cholesterol_v_", "triglycerides_v_",
                  "ldl_cholesterol_v_", "chol_hdlc_ratio_v_", "non_hdl_cholesterol_v_", "glucose_v_", "c-reactive_protein_v_", "insulin_v_"]
...
for file in files:
    ...
    attributes = ["CHOLESTEROL, TOTAL", "HDL CHOLESTEROL", "TRIGLYCERIDES", "LDL-CHOLESTEROL",
                  "CHOL/HDLC RATIO", "NON HDL CHOLESTEROL", "GLUCOSE", "C-REACTIVE PROTEIN", "INSULIN"]
    ...
    Id = (final_list[final_list.index('Specimen:') -
                     1].split(':')[1].strip())[0:-1]
    visit_num = (final_list[final_list.index(
        'Specimen:')-1].split(':')[1].strip())[-1]
    ...
    columns = [sheet.cell(
        row=1, column=i).value for i in range(1, max_col + 1)]

    
    Id_index = isPresent(Id, Id_list)
    column_num = isPresent("cholesterol_v_" + visit_num, columns)
        
    if Id_index == -1:  # If this is patient's first time visit
        ...
        if column_num == -1:
            for i in range(1, len(keys)):
                sheet.cell(row=1, column=max_col +
                               i).value = keys[i]+visit_num
